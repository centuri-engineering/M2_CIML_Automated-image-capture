...
starting_loops = np.arange(0,total_duration+1,elapse_time) # Times when loops will start
...
#For Y
Ycoord_along_Y_single =  np.arange(0,nb_well_along_Y*dish_move, dish_move) # Coordinates in Y for a single line
Ycoord_zigzag_dish = np.reshape(np.transpose(np.tile(Ycoord_along_Y_single, (nb_well_along_X,1))), (1,nb_well)) #Full coordinates in Y for 1 petri dish 
#For X
Xcoord_along_X_single = np.arange(0,nb_well_along_X*dish_move, dish_move) # Coordinates in X for a single line
Xcoord_along_X_goback = np.concatenate((Xcoord_along_X_single, Xcoord_along_X_single[::-1]), axis=None) # To use a zig-zag pattern 
Xcoord_zigzag_dish = np.tile(Xcoord_along_X_goback, (1,(nb_well_along_Y)))[:,0:nb_well] # Full coordinates in X for 1 petri dish
...
#For X
Xboxcoord_along_X_single = np.arange(0,nb_box_along_X*box_move_along_X, box_move_along_X) # Coordinates in X for a single line of box
Xboxcoord_along_X_goback = np.concatenate((Xboxcoord_along_X_single, Xboxcoord_along_X_single[::-1]), axis=None) # To use a zig-zag pattern 
Xboxcoord_zigzag_box = np.tile(Xboxcoord_along_X_goback, (1,nb_box_along_Y))[:,0:nb_box] # Full coordinates in X for all the boxes
#For Y
Yboxcoord_along_Y =  np.arange(0,nb_box_along_Y*box_move_along_Y, box_move_along_Y) # Coordinates in Y for a single line of box
Yboxcoord_zigzag_box = np.reshape(np.transpose(np.tile(Yboxcoord_along_Y, (nb_box_along_X,1))), (1,nb_box)) # Full coordinates in Y for all the boxes
...
for it0,start_loop in enumerate(starting_loops): # Loops to take the pictures
    if it0!=0: # This statetment is here because the timer starts before the loop
        while it0!=0 and (int(time.time())-int(start))!=int(start_loop): # As long as the timer has not reached the desired duration, the loop is stuck
            pass  
    for it1 in range(nb_box): # Loop to define the coordinates of each box
        for it2 in range(nb_well): # Loop to define the coordinates of each well of the Petri dish
            s.write(b'G0 X' + str(Xboxcoord_zigzag_box[0,it1] + Xcoord_zigzag_dish[0,it2]).encode() + # Send x g-code coord block to grbl 
            b' Y' + str(Yboxcoord_zigzag_box[0,it1] + Ycoord_zigzag_dish[0,it2]).encode() + b'\n') # Send Y g-code coord block to grbl 
